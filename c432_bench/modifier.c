...
void addgate(char *gate,char *position,char *key)
{
    ...
    srcFile = fopen("tempc432.bench","r+");
    dstFile = fopen("dubc432.bench","a");
    char ch[50];
    char temp[10];
...
int main()
{
    copyfiles("tempc432.bench","c432.bench");
    FILE *ptr = fopen("c432nodeslist.txt","r");
    char key[10] = "";
    char ch;
    int i=-1;
    ch = fgetc(ptr);
    while(ch!=EOF)
    {
        if(ch == '\'')
        {
            ...
            copyfiles("tempc432.bench","dubc432.bench");
            FILE *empty = fopen("dubc432.bench","w");
            fclose(empty);
        }
        ch=fgetc(ptr);
    }
