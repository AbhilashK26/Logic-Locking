...
void addgate(char *gate,char *position,char *key)
{
    ...
    srcFile = fopen("tempc17.bench","r+");
    dstFile = fopen("dubc17.bench","a");
    char ch[50];
    char temp[10];
...
int main()
{
    copyfiles("tempc17.bench","c17.bench");
    FILE *ptr = fopen("c17nodeslist.txt","r");
    char key[10] = "";
    char ch;
    int i=-1;
    ch = fgetc(ptr);
    while(ch!=EOF)
    {
        if(ch == '\'')
        {
            ...
            copyfiles("tempc17.bench","dubc17.bench");
            FILE *empty = fopen("dubc17.bench","w");
            fclose(empty);
        }
        ch=fgetc(ptr);
    }
