...
void addgate(char *gate,char *position,char *key)
{
    ...
    srcFile = fopen("tempc1908.bench","r+");
    dstFile = fopen("dubc1908.bench","a");
    char ch[50];
    char temp[10];
...
int main()
{
    copyfiles("tempc1908.bench","c1908.bench");
    FILE *ptr = fopen("c1908nodeslist.txt","r");
    char key[10] = "";
    char ch;
    int i=-1;
    ch = fgetc(ptr);
    while(ch!=EOF)
    {
        if(ch == '\'')
        {
            ...
            copyfiles("tempc1908.bench","dubc1908.bench");
            FILE *empty = fopen("dubc1908.bench","w");
            fclose(empty);
        }
        ch=fgetc(ptr);
    }
