...
void addgate(char *gate,char *position,char *key)
{
    ...
    srcFile = fopen("tempc499.bench","r+");
    dstFile = fopen("dubc499.bench","a");
    char ch[50];
    char temp[10];
...
int main()
{
    copyfiles("tempc499.bench","c499.bench");
    FILE *ptr = fopen("c499nodeslist.txt","r");
    char key[10] = "";
    char ch;
    int i=-1;
    ch = fgetc(ptr);
    while(ch!=EOF)
    {
        if(ch == '\'')
        {
            ...
            copyfiles("tempc499.bench","dubc499.bench");
            FILE *empty = fopen("dubc499.bench","w");
            fclose(empty);
        }
        ch=fgetc(ptr);
    }
