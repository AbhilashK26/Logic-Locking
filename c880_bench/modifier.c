...
void addgate(char *gate,char *position,char *key)
{
    ...
    srcFile = fopen("tempc880.bench","r+");
    dstFile = fopen("dubc880.bench","a");
    char ch[50];
    char temp[10];
...
int main()
{
    copyfiles("tempc880.bench","c880.bench");
    FILE *ptr = fopen("c880nodeslist.txt","r");
    char key[10] = "";
    char ch;
    int i=-1;
    ch = fgetc(ptr);
    while(ch!=EOF)
    {
        if(ch == '\'')
        {
            ...
            copyfiles("tempc880.bench","dubc880.bench");
            FILE *empty = fopen("dubc880.bench","w");
            fclose(empty);
        }
        ch=fgetc(ptr);
    }
